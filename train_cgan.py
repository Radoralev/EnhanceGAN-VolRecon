...
parser.add_argument('--ckpt_netG', type=str, required=False,
                    help='Checkpoint for generator network')
parser.add_argument('--ckpt_netD', type=str, required=False,
                    help='Checkpoint for discriminator network')
parser.add_argument('--run_name', type=str, required=False, help='Run name')
parser.add_argument('--epoch_old', type=int, required=False,
                    help='Epoch of checkpoint (necessary for logging).')
...
dataset_train = CGanTrainDataset(
    root_dir1, root_dir2, opt=pix2pix_options, isTrain=True)
...
for epoch in range(0 if not args.epoch_old else int(args.epoch_old), num_epochs):
    ...
    for i, data in tqdm(enumerate(data_loader)):
        ...
        if i % 28 == 0:
            ...
            writer.add_scalar('Train/Epoch_{}/Generator Loss (VGG)',
                              pix2pix_model.loss_G_VGG, global_step=epoch * len(data_loader) + i)
            writer.add_scalar('Train/Epoch_{}/Generator Loss (MSE)',
                              pix2pix_model.loss_G_GAN_Feat, global_step=epoch * len(data_loader) + i)
            writer.add_scalar('Train/Epoch_{}/Generator Loss (L1)',
                              pix2pix_model.loss_G_L1, global_step=epoch * len(data_loader) + i)
            writer.add_scalar('Train/Epoch_{}/Generator Loss',
                              pix2pix_model.loss_G, global_step=epoch * len(data_loader) + i)
            writer.add_scalar('Train/Epoch_{}/D-Fake Loss',
                              pix2pix_model.loss_D_fake, global_step=epoch * len(data_loader) + i)
            writer.add_scalar('Train/Epoch_{}/D-Real Loss',
                              pix2pix_model.loss_D_real, global_step=epoch * len(data_loader) + i)
            writer.add_scalar('Train/Epoch_{}/Discriminator Loss',
                              pix2pix_model.loss_D, global_step=epoch * len(data_loader) + i)
            ...
            writer.add_image(
                'Train/Epoch_{}/Image_{}'.format(epoch, i), img_grid, global_step=epoch)
    ...
    with torch.no_grad():  # do not need to compute gradients when evaluating
        for i, data in tqdm(enumerate(data_loader_test)):
            ...
            writer.add_scalar('Test/Epoch_{}/Generator Loss', pix2pix_model.loss_G,
                              global_step=epoch * len(data_loader_test) + i)
            writer.add_scalar('Test/Epoch_{}/Discriminator Loss',
                              pix2pix_model.loss_D, global_step=epoch * len(data_loader_test) + i)
            ...
            if (epoch+1) % 5 == 0:
                ...
                img_grid = make_grid(img_grid, nrow=3)
                writer.add_image(
                    'Test/Epoch_{}/Image_{}'.format(epoch, i), img_grid, global_step=epoch)
        ...
        torch.save(pix2pix_model.netD.state_dict(),
                   f'checkpoints_cgan/{run_name}/checkpoint_{epoch+1}_netD.pth')
        torch.save(pix2pix_model.netG.state_dict(),
                   f'checkpoints_cgan/{run_name}/checkpoint_{epoch+1}_netG.pth')
