...
def calculate_metrics(real_A, real_B, fake_B, device):
    ...
    psnr_gt_output = peak_signal_noise_ratio(real_B_np, fake_B_np, data_range=1)
    ssim_input_gt = structural_similarity(real_A_np, real_B_np, multichannel=True, data_range=1)
    ssim_gt_output = structural_similarity(real_B_np, fake_B_np, multichannel=True, data_range=1)
...
def main(ckpt_netG, ckpt_netD, out):
    ...
    data_loader_test = DataLoader(dataset_test, batch_size=1, shuffle=False, num_workers=8)
    ...
    with torch.no_grad():  
        for i, data in tqdm(enumerate(data_loader_test)):
            ...
            writer.add_image('Test/Image_{}'.format(i), img_grid, global_step=i)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ckpt_netG', type=str, required=True, help='Checkpoint for generator network')
    parser.add_argument('--ckpt_netD', type=str, required=True, help='Checkpoint for discriminator network')
